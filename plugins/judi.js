...
async function handler(m, { conn, args, isROwner }) {
    ...
    try {
        ...
        if ((user.money * 1) < count) return conn.sendBut(m.chat, '💵Uang kamu tidak cukup!!', wm, 'Kerja', `.kerja`, m)
        if (!(m.sender in confirm)) {
            ...
            conn.send2But(m.chat, `*❗ Warning ❗*\nJangan judi karena tidak akan menang, BENERAN!!\nApakah anda yakin  mau melakukan judi?\n⏲️ *Timeout* : 60s`, wm, '✔️', `ya`, '✖️', `no`, m)
        }
    } catch (e) {
        if (m.sender in confirm) {
...
handler.before = async m => {
    ...
    try {
        if (/^(yes|ya|✔️)?$/i.test(txt)) {
            let Bot = (Math.ceil(Math.random() * 100)) * 1
            let Kamu = (Math.floor(Math.random() * 26)) * 1
            let status = 'Kalah'
            if (Bot < Kamu) {
                user.money += count * 1
                status = 'Menang'
...
▸ Kamu *${status}*, kamu ${status == 'Menang' ? `Mendapatkan *+${count * 2}*` : status == 'Kalah' ? `Kehilangan *-${count * 1}*` : `Mendapatkan *+${Math.floor(count / 1.5)}*`} Money 💹
    `.trim())
            ...
            delete confirm[m.sender]
            return !0
        } else if (/^(no|tidak|ga|✖️)?$/i.test(txt)) {
            clearTimeout(timeout)
            delete confirm[m.sender]
            m.reply('Rejected')
            return !0
