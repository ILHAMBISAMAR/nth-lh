        ...
        try {
            ...
            try {
                ...
                if (user) {
                    ...
                    if (!isNumber(user.voucher)) user.voucher = 0
             ...
             if (!isNumber(user.ayam)) user.ayam = 0
                    ...
                    if (!isNumber(user.lastmancingeasy)) user.lastfishing = 0
            ...
            for (let name in global.plugins) {
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                        this.reply(m.chat, `*_Limit anda habis,_* *_silahkan ketik .limit,_* *_anda bisa membeli limit dg exp & money melalui ${usedPrefix}buy jumlah limit atau ${usedPrefix}bely jumlah limit_*`, m)
                    ...
                    for (let user of participants) {
                       let pp = './src/welcome.jpg'
                        try {
                            ...
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || `Beban join @user` || 'BEBAN JOIN @user').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc ? String.fromCharCode(8206).repeat(4001) + groupMetadata.desc : '') :
                                (chat.sBye || this.bye || `Beban leave @user` || 'BEBAN LEFT @user'))
                            let wel = `https://api.lolhuman.xyz/api/welcomeimage?apikey=LuckyBotz&img=${pp}&text=${await this.getName(user)}`
                            
                            let lea = `https://api.lolhuman.xyz/api/base/leave?apikey=LuckyBotz&img1=${pp}&img2=${ await this.profilePictureUrl(id, 'image')}&background=https://telegra.ph/file/52fcaaf5dbd0c8ded9a2a.jpg&username=${await this.getName(user)}&member=${groupMetadata.participants.length}&groupname=${await this.getName(id)}`
                                this.sendButtonImg(id, action === 'add' ? wel : lea, text, '', "Jangan Dipencet!🗿", "huuu", null)
                                }
                    }
                }
                break
